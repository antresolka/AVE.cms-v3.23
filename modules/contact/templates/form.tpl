...
<div id="module_content">
	...
	<form method="post" enctype="multipart/form-data" onSubmit='return checkForm(this)'>
		...
		{else}
		<input type="text" value="{$smarty.request.contact_form_in_email|default:$smarty.session.user_email|stripslashes|escape}" name="contact_form_in_email" id="contact_form_in_email" style="width:215px;" {literal}check_pattern="^[\w-]+(\.[\w-]+)*@([\w-]+\.)+[a-zA-Z]{2,7}$"{/literal} check_message="{#CONTACT_CHECK_EMAIL#}" />
		{/if}
		...
		{foreach from=$fields item=field}
			...
			<div class="mod_contact_field" style="float:left; width:{$field->contact_field_size|default:'300'}px;">
				...
				<div class="mod_contact_right">
					...
					{elseif $field->contact_field_type == 'title'}
						<h2>{$field->contact_field_value|escape}</h2>
