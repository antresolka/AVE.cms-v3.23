...
//		$text = htmlspecialchars($text);
	 ...
	 */
	function _contactThankyou($tpl_dir, $lang_file,$contact_form_id='')
	{
		global $AVE_Template;

		$AVE_Template->config_load($lang_file);
		$contact_form_id=($contact_form_id>'' ? $contact_form_id : intval($_REQUEST['contact_form_id']));

		if (file_exists($tpl_dir . 'thankyou-'.$contact_form_id.'.tpl')) {
			$AVE_Template->display($tpl_dir . 'thankyou-'.$contact_form_id.'.tpl');
		} else {
	...
	function _contactFileUpload($maxupload = '0')
	{
		...
		$blacklist = array("php", "phtml", "php3", "php4", "js", "pl");
		if (isset($_FILES['upfile']) && is_array($_FILES['upfile']))
		{
			for ($i=0; $i<count($_FILES['upfile']['tmp_name']); $i++)
			{
				...
				//var_dump($ext);
				//var_dump(!in_array($ext,$blacklist));
				if ($_FILES['upfile']['tmp_name'][$i] != '' && !in_array($ext,$blacklist))
				{
 ...
 */
	...
	function contactFormShow($tpl_dir, $lang_file, $contact_form_id, $spam_protect = null, $max_upload = null, $fetch = '0')
	{
		...
		$row = $AVE_DB->Query("
			SELECT *
			FROM " . PREFIX . "_module_contacts
			WHERE Id = '" . $contact_form_id . "'
		")->FetchRow();

		$GLOBALS["user_header"]['contac_form_'.$contact_form_id]=$row->contact_form_message_scripts;
		...
		else
		{
			...
			$sql = $AVE_DB->Query("
				SELECT *
				FROM " . PREFIX . "_module_contact_fields
		...
		//файл шаблона формы
		$tplf=file_exists($tpl_dir . 'form-'.$contact_form_id.'.tpl') ? $tpl_dir . 'form-'.$contact_form_id.'.tpl' : $tpl_dir . 'form.tpl';
		// Возвращаем сформированную контактную форму
		if ($fetch == 1) return $AVE_Template->fetch($tplf);

		// Отображаем сформированную контактную форму
		$AVE_Template->display($tplf);
	}
	 ...
	 * @param int $max_upload максимальный размер прикреплённого файла
	 */
	function contactFormSend($tpl_dir, $lang_file, $contact_form_id, $spam_protect = null, $max_upload = null, $extemail = null)
	{
		...
		// Обрабатываем email-ы из тега
	 	$extemail = trim($extemail);
		if ($extemail)
		{
			$extemail = explode(',',$extemail);
			foreach ($extemail as $email)
			{
				$email = trim($email);
				if ($email && strpos($email,'@') && strpos($email,'.')) $extemail_new[] = $email;
			}
		}
		...
		$contact_form_in_attachment = str_ireplace(BASE_DIR .'/' .ATTACH_DIR.'/','',(is_array($attach) && count($attach) >= 0) ? implode(';', $attach) : '');
		$rcvs=explode(',',$row->contact_form_reciever);
		if(!empty($extemail_new))$rcvs = array_merge($rcvs, $extemail_new);
		foreach($rcvs as $rcv){
			send_mail(
				$rcv,
				stripslashes(mb_substr($text, 0, $row->contact_form_mail_max_chars)),
				$_POST['contact_form_in_subject'],
				empty($_POST['contact_form_in_email']) ? get_settings('mail_from') : $_POST['contact_form_in_email'],
				empty($_POST['contact_form_in_email']) ? get_settings('mail_from') : $_POST['contact_form_in_email'],
				'text',
				$attach
			);
		...
		$AVE_DB->Query("
			INSERT
			INTO " . PREFIX . "_module_contact_info
			SET
		...
		");
		$iid=$AVE_DB->InsertId();
		reportLog('Контакты - '.$_SESSION['user_name'] . ' заполнил <a href="javascript:void(0);" onclick="cp_pop(\'index.php?do=modules&action=modedit&mod=contact&moduleaction=showmessages_new&sub=view&id='.$iid.'&pop=1&reply=no\',\'\',\'\',\'1\',\'modcontactedit\');">форму</a>', 2, 2);

		// Отображаем сообщение о успешной отправке формы с благодарностью
		$this->_contactThankyou($tpl_dir, $lang_file,$contact_form_id);
	}
	 ...
	 *
	 * @param string $adm_dir путь к папке с шаблонами
	 */
	function contactFormList($adm_dir)
	{
		global $AVE_DB, $AVE_Template;

		// Получаем общее количество контактных форм в системе
		$num = $AVE_DB->Query("SELECT COUNT(*) FROM " . PREFIX . "_module_contacts")->GetCell();
		...
		$sql = $AVE_DB->Query("
			...
			FROM
				" . PREFIX . "_module_contacts AS frm
			LEFT OUTER JOIN
				" . PREFIX . "_module_contact_info
	 ...
	 * @param string $adm_dir путь к папке с шаблонами
	 */
	function contactFormNew($adm_dir)
	{
		global $AVE_DB, $AVE_Template;
		...
		switch($_REQUEST['sub'])
		{
			...
			case '':
				...
				$AVE_Template->assign('include_path', $adm_dir);
				$AVE_Template->assign('formaction', 'index.php?do=modules&action=modedit&mod=contact&moduleaction=new&sub=save&cp=' . SESSION . '');
			...
			case 'save':
				...
				header('Location:index.php?do=modules&action=modedit&mod=contact&moduleaction=edit&id=' . $iid . '&cp=' . SESSION);
				exit;
		}
	}
	...
	function contactFormFieldSave($contact_form_id)
	{
		...
		if (!empty($_POST['contact_field_title']))
		{
			...
			$AVE_DB->Query("
				INSERT
				INTO " . PREFIX . "_module_contact_fields
				SET
					Id					  = '',
		...
		// Выполняем обновление страницы
		header('Location:index.php?do=modules&action=modedit&mod=contact&moduleaction=edit&id=' . $contact_form_id . '&cp=' . SESSION);
	}
	 ...
	 * @param int $contact_form_id идентификатор формы
	 */
	function contactFormEdit($adm_dir, $contact_form_id)
	{
		global $AVE_DB, $AVE_Template;
		...
		$AVE_Template->assign('items', $items);
		$AVE_Template->assign('include_path', $adm_dir);
		$AVE_Template->assign('formaction', 'index.php?do=modules&action=modedit&mod=contact&moduleaction=save&cp=' . SESSION . '&id=' . $_REQUEST['id'] . '');
		$AVE_Template->assign('content', $AVE_Template->fetch($adm_dir . 'admin_fields.tpl'));
	}
	...
	function contactFormSave($contact_form_id)
	{
		...
		$AVE_DB->Query("
			UPDATE " . PREFIX . "_module_contacts
			SET
				contact_form_title			= '" . $_REQUEST['contact_form_title'] . "',
				contact_form_mail_max_chars   = '" . $_REQUEST['contact_form_mail_max_chars'] . "',
				contact_form_reciever		 = '" . $_REQUEST['contact_form_reciever'] . "',
		...
		foreach ($_POST['contact_field_title'] as $id => $field)
		{
			...
			if (!empty($field))
			{
				...
				$AVE_DB->Query("
					UPDATE " . PREFIX . "_module_contact_fields
					SET
						contact_field_title	 = '" . $contact_field_title . "',
	...
	function contactFormDelete($contact_form_id)
	{
		...
		$row = $AVE_DB->Query("
			...
			FROM " . PREFIX . "_module_contact_info
			WHERE contact_form_id = '" . $contact_form_id . "'
		")->FetchRow();
	 ...
	 */
	function contactMessageShow($adm_dir, $contact_form_id, $newold = '')
	{
		global $AVE_DB, $AVE_Template;
		...
		switch($_REQUEST['sub'])
		{
			...
			case '':
				...
				$AVE_Template->assign('items', $items);
				$AVE_Template->assign('content', $AVE_Template->fetch($adm_dir . 'admin_messages.tpl'));
				break;
			...
			case 'view':
				...
				// Отображаем данные
				$AVE_Template->assign('content', $AVE_Template->fetch($adm_dir . 'admin_messageform.tpl'));
				break;
		}
	}
	...
	function contactMessageReply()
	{
		...
		$AVE_DB->Query("
			UPDATE " . PREFIX . "_module_contact_info
			SET
	...
	function contactAttachmentDelete()
	{
		...
		foreach ($_REQUEST['del'] as $id => $del)
		{
			if ($this->_delfile == 1)
			{
		...
		// Выполняем обновление страницы
		header('Location:index.php?do=modules&action=modedit&mod=contact&moduleaction=showmessages_new&id='.$_REQUEST["id"].'&cp=' . SESSION);
		exit;
	}
	/**
	 * Метод, удаления прикреплённых файлов
