		...
		{foreach from=$items item=item}
			<tr>
				...
				<td width="1%" align="center">
					{if $smarty.request.moduleaction=='showmessages_new'}
						<a class="icon_sprite ico_look topleftDir" title="{#CONTACT_SHOW_ANSWER#}" href="javascript:void(0);" onclick="windowOpen('index.php?do=modules&action=modedit&mod=contact&moduleaction=showmessages_new&sub=view&cp={$sess}&id={$item->Id}&pop=1','','','1','modcontactedit');"></a>
					{else}
						<a class="icon_sprite ico_look topleftDir" title="{#CONTACT_SHOW_ANSWER#}" href="javascript:void(0);" onclick="windowOpen('index.php?do=modules&action=modedit&mod=contact&moduleaction=showmessages_new&sub=view&cp={$sess}&id={$item->Id}&pop=1&reply=no','','','1','modcontactedit');"></a>
					{/if}
				</td>
				<td width="1%" align="center">
						<a class="icon_sprite ico_delete topleftDir ConfirmDelete" dir="{#CONTACT_DELETE_MESS#}" name="{#CONTACT_DELETE_CONFIRM2#}" title="{#CONTACT_DELETE_MESS#}" href="index.php?do=modules&action=modedit&mod=contact&moduleaction=delete_message&del={$item->Id}&id={$smarty.request.id}&cp={$sess}"></a>
				</td>
			</tr>
		{/foreach}
			{if $items}
